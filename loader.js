	...
	nRelate = function() {		
		var _nrelate = {
			...
			init : function( args ) {
				...
				this.options.plugins = sanitized_plugins || { related: this.extend(true, {}, this.plugin_helpers[ 'related' ]._defaults, this.plugin_helpers[ 'related' ] ) };
			...
			sctc : function( plugin, ct, args ) {
				...
				if ( ct.settings.cssstyle ) {
					...
					} else if ( ct.settings.cssstyle != 'none' ) {
				...
				for( p in this.options.plugins ) {
					if ( this.is_array( this.options.plugins[ p ].cts ) ) {
						for ( i=0,l=this.options.plugins[ p ].cts.length; i<l; i++ ) {
							...
							if ( ct.settings.count_views && !ct.settings.viewed && this.xHeight( ct ) ) {
			...
			*/
			...
			tuc : function( ct, plugin, args ) {
				...
				if ( !( link = this.xgebcn( "nr_uc_toggle", "div", ct )[0] ) ) {
					...
					desc.innerHTML = '<p class="uc_on_msg">  Click to turn on user control</p>' + 
									 ...
									 '<p class="uc_desc">Our user control tool filters out content you dislike, and puts you in charge of what you read. Simply &lsquo;x&rsquo; out items you don&rsquo;t wish to see, and they will no longer pop up on this network. Then, enjoy quality content within a personalized user experience designed by you!</p>';
			...
			cab : function( ct ) {
				...
				if ( ct.settings.blocked_units_reported || ( ct.settings.plugin == 'flyout' && !this.xhc( ct, "nr_fo_opened" ) ) ) {
					// Ignore callback when bl call was made or widget is a closed FO to prevent false positives
					return false;
				}
				...
				ct.settings.blocked_units_reported = true;
			...
			constants : {
				...
				uc_url					: "http://api.nrelate.com/common/nr_uc_update.php",
...
<<<<<<< Updated upstream
		...
		var _plugin_base = {
			...
			supported_cssstyles : [null, 'default', 'bty', 'huf', 'tre', 'toon', 'pol', 'none', 'custom'],
		...
		_nrelate.plugin_helpers = {
			...
			related : _nrelate.extend(true, {}, _plugin_base, {
				api_url		: "http://api.nrelate.com/rcw_js/" + _nrelate.constants.version + "/",
				shortname 	: "rc",
				fullname 	: "related"
			}),
			...
			flyout : _nrelate.extend(true, {}, _plugin_base, {
				api_url			: "http://api.nrelate.com/fow_js/" + _nrelate.constants.version + "/",
				shortname 		: "fo",
				fullname 		: "flyout",
				home_page_only 	: true,
		...
		mDialog.prototype.hide = function( dialogOnly )
		{
			...
			if (e) {
				...
				// remove and reset overlay, dialog instances
...
>>>>>>> Stashed changes
		...
		return {
			...
			is_mobile : function() {
				return _nrelate.is_mobile.apply( _nrelate, Array.prototype.slice.call(arguments) );
			},
